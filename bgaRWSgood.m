...
for gen = 1:nGenerations
    ...
    for i = 1:2:popSize-1
        if rand < crossoverRate
            ...
            p1 = newPop(i, :);
            p2 = newPop(i+1, :);
            newPop(i, :) = [p1(1:point), p2(point+1:end)];
            newPop(i+1, :) = [p2(1:point), p1(point+1:end)];
        end
    end
    ...
    % === Insert Elites ===
    newPop(1:nElite, :) = elites;  % 替代前 nElite 個位置
...
title('Best Fitness Over Generations(Keep greatest)');
